        ...
        if ($this->loginFailureCountGreater($this->settingsService->getByPath('failedTries'))) {
            ...
            if (empty($data['challenge']) || empty($data['response'])) {
                ...
                if ($this->writeAttemptLog) {
                    ...
                    t3lib_div::sysLog(
                        sprintf("Login-attempt from %s (%s), username '%s', captcha was empty!",
                            $this->authInfo['REMOTE_ADDR'], $this->authInfo['REMOTE_HOST'],
                            $this->login['uname']),
                        'Core',
                        0
                    );
                ...
                if (!$response || strtolower($response[0]) == 'false') {
                    ...
                    if ($this->writeAttemptLog) {
                        ...
                        t3lib_div::sysLog(
                            sprintf("Login-attempt from %s (%s), username '%s', captcha was mistaken!",
                                $this->authInfo['REMOTE_ADDR'], $this->authInfo['REMOTE_HOST'],
                                $this->login['uname']),
                            'Core',
                            0
